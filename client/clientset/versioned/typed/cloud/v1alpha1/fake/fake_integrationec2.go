/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-wavefront-api/apis/cloud/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeIntegrationEc2s implements IntegrationEc2Interface
type FakeIntegrationEc2s struct {
	Fake *FakeCloudV1alpha1
	ns   string
}

var integrationec2sResource = schema.GroupVersionResource{Group: "cloud.wavefront.kubeform.com", Version: "v1alpha1", Resource: "integrationec2s"}

var integrationec2sKind = schema.GroupVersionKind{Group: "cloud.wavefront.kubeform.com", Version: "v1alpha1", Kind: "IntegrationEc2"}

// Get takes name of the integrationEc2, and returns the corresponding integrationEc2 object, and an error if there is any.
func (c *FakeIntegrationEc2s) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.IntegrationEc2, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(integrationec2sResource, c.ns, name), &v1alpha1.IntegrationEc2{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.IntegrationEc2), err
}

// List takes label and field selectors, and returns the list of IntegrationEc2s that match those selectors.
func (c *FakeIntegrationEc2s) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.IntegrationEc2List, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(integrationec2sResource, integrationec2sKind, c.ns, opts), &v1alpha1.IntegrationEc2List{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.IntegrationEc2List{ListMeta: obj.(*v1alpha1.IntegrationEc2List).ListMeta}
	for _, item := range obj.(*v1alpha1.IntegrationEc2List).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested integrationEc2s.
func (c *FakeIntegrationEc2s) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(integrationec2sResource, c.ns, opts))

}

// Create takes the representation of a integrationEc2 and creates it.  Returns the server's representation of the integrationEc2, and an error, if there is any.
func (c *FakeIntegrationEc2s) Create(ctx context.Context, integrationEc2 *v1alpha1.IntegrationEc2, opts v1.CreateOptions) (result *v1alpha1.IntegrationEc2, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(integrationec2sResource, c.ns, integrationEc2), &v1alpha1.IntegrationEc2{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.IntegrationEc2), err
}

// Update takes the representation of a integrationEc2 and updates it. Returns the server's representation of the integrationEc2, and an error, if there is any.
func (c *FakeIntegrationEc2s) Update(ctx context.Context, integrationEc2 *v1alpha1.IntegrationEc2, opts v1.UpdateOptions) (result *v1alpha1.IntegrationEc2, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(integrationec2sResource, c.ns, integrationEc2), &v1alpha1.IntegrationEc2{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.IntegrationEc2), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeIntegrationEc2s) UpdateStatus(ctx context.Context, integrationEc2 *v1alpha1.IntegrationEc2, opts v1.UpdateOptions) (*v1alpha1.IntegrationEc2, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(integrationec2sResource, "status", c.ns, integrationEc2), &v1alpha1.IntegrationEc2{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.IntegrationEc2), err
}

// Delete takes name of the integrationEc2 and deletes it. Returns an error if one occurs.
func (c *FakeIntegrationEc2s) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(integrationec2sResource, c.ns, name), &v1alpha1.IntegrationEc2{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeIntegrationEc2s) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(integrationec2sResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.IntegrationEc2List{})
	return err
}

// Patch applies the patch and returns the patched integrationEc2.
func (c *FakeIntegrationEc2s) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.IntegrationEc2, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(integrationec2sResource, c.ns, name, pt, data, subresources...), &v1alpha1.IntegrationEc2{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.IntegrationEc2), err
}
