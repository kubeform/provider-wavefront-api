/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-wavefront-api/apis/cloud/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeIntegrationAzures implements IntegrationAzureInterface
type FakeIntegrationAzures struct {
	Fake *FakeCloudV1alpha1
	ns   string
}

var integrationazuresResource = schema.GroupVersionResource{Group: "cloud.wavefront.kubeform.com", Version: "v1alpha1", Resource: "integrationazures"}

var integrationazuresKind = schema.GroupVersionKind{Group: "cloud.wavefront.kubeform.com", Version: "v1alpha1", Kind: "IntegrationAzure"}

// Get takes name of the integrationAzure, and returns the corresponding integrationAzure object, and an error if there is any.
func (c *FakeIntegrationAzures) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.IntegrationAzure, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(integrationazuresResource, c.ns, name), &v1alpha1.IntegrationAzure{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.IntegrationAzure), err
}

// List takes label and field selectors, and returns the list of IntegrationAzures that match those selectors.
func (c *FakeIntegrationAzures) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.IntegrationAzureList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(integrationazuresResource, integrationazuresKind, c.ns, opts), &v1alpha1.IntegrationAzureList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.IntegrationAzureList{ListMeta: obj.(*v1alpha1.IntegrationAzureList).ListMeta}
	for _, item := range obj.(*v1alpha1.IntegrationAzureList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested integrationAzures.
func (c *FakeIntegrationAzures) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(integrationazuresResource, c.ns, opts))

}

// Create takes the representation of a integrationAzure and creates it.  Returns the server's representation of the integrationAzure, and an error, if there is any.
func (c *FakeIntegrationAzures) Create(ctx context.Context, integrationAzure *v1alpha1.IntegrationAzure, opts v1.CreateOptions) (result *v1alpha1.IntegrationAzure, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(integrationazuresResource, c.ns, integrationAzure), &v1alpha1.IntegrationAzure{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.IntegrationAzure), err
}

// Update takes the representation of a integrationAzure and updates it. Returns the server's representation of the integrationAzure, and an error, if there is any.
func (c *FakeIntegrationAzures) Update(ctx context.Context, integrationAzure *v1alpha1.IntegrationAzure, opts v1.UpdateOptions) (result *v1alpha1.IntegrationAzure, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(integrationazuresResource, c.ns, integrationAzure), &v1alpha1.IntegrationAzure{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.IntegrationAzure), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeIntegrationAzures) UpdateStatus(ctx context.Context, integrationAzure *v1alpha1.IntegrationAzure, opts v1.UpdateOptions) (*v1alpha1.IntegrationAzure, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(integrationazuresResource, "status", c.ns, integrationAzure), &v1alpha1.IntegrationAzure{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.IntegrationAzure), err
}

// Delete takes name of the integrationAzure and deletes it. Returns an error if one occurs.
func (c *FakeIntegrationAzures) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(integrationazuresResource, c.ns, name), &v1alpha1.IntegrationAzure{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeIntegrationAzures) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(integrationazuresResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.IntegrationAzureList{})
	return err
}

// Patch applies the patch and returns the patched integrationAzure.
func (c *FakeIntegrationAzures) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.IntegrationAzure, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(integrationazuresResource, c.ns, name, pt, data, subresources...), &v1alpha1.IntegrationAzure{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.IntegrationAzure), err
}
